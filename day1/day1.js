...
const findFirstOccurrence = (input, textNumber, firstOccurrence) => {
    const indexOfOccurrence = input.indexOf(textNumber);

    if (indexOfOccurrence !== -1 && (indexOfOccurrence < firstOccurrence.index || firstOccurrence.index === undefined)) {
        return { index: indexOfOccurrence, number: textToNumberMap[textNumber] };
    }

    return undefined;
}

const findLastOccurrence = (input, textNumber, firstOccurrence) => {
    const indexOfOccurrence = input.lastIndexOf(textNumber);
...
const getFirstTextNumber = (input) => {
    const firstOccurrence = {};
    Object.keys(textToNumberMap).forEach(textNumber => {
        const result = findFirstOccurrence(input, textNumber, firstOccurrence);
        if (result !== undefined) {
            firstOccurrence.index = result.index;
            firstOccurrence.number = result.number;
        }
    });

    return firstOccurrence.index !== undefined ? firstOccurrence : undefined;
}

const getLastTextNumber = (input) => {
    const lastOccurrence = {};
    Object.keys(textToNumberMap).forEach(textNumber => {
        const result = findLastOccurrence(input, textNumber, lastOccurrence);
        if (result !== undefined) {
            lastOccurrence.index = result.index;
            lastOccurrence.number = result.number;
        }
    });

    return lastOccurrence.index !== undefined ? lastOccurrence : undefined;
}


const findNumbersFromInput = (input) => {
    let firstNumber = undefined;
    let lastNumber = undefined;

    const firstTextNumber = getFirstTextNumber(input);
    const lastTextNumber = getLastTextNumber(input);

    for (let charPointer = 0; charPointer <= input.length; charPointer++ ) {
        if (firstTextNumber && charPointer >= firstTextNumber.index) {
            firstNumber = firstTextNumber.number;
            break;
        }
        const char = input[charPointer];
    ...
    for (let charPointer = input.length-1; charPointer >= 0; charPointer-- ) {
        if (lastTextNumber && charPointer <= lastTextNumber.index) {
            lastNumber = lastTextNumber.number;
            break;
        }
        const char = input[charPointer];
        if(!isNaN(parseFloat(char))) {
...
inputList.forEach(input => {
    const {firstNumber, lastNumber} = findNumbersFromInput(input);

    if (firstNumber === undefined || lastNumber === undefined) {
        throw Error(`No number found! first: ${firstNumber} last: ${lastNumber}`)
    }

    const answerForInput = `${firstNumber}${lastNumber}`;
    answer += parseFloat(answerForInput);
})
